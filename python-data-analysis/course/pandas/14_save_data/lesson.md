...
def demo_save_and_reload(path: Path) -> pd.DataFrame:
    """演示创建父目录、无索引保存，再重新读取。"""
    table = pd.DataFrame({"item": ["笔", "本"], "quantity": [3, 5]})
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    with TemporaryDirectory() as directory:
        target = Path(directory) / "out" / "items.csv"
        reloaded = demo_save_and_reload(target)
        print(target.exists())
        print(reloaded.to_dict("records"))
