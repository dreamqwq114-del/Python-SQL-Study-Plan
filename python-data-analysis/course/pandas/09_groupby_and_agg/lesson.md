...
def demo_group_count() -> pd.DataFrame:
    """按是否流失分组统计行数与平均年龄，仅演示 agg 的命名聚合写法。"""
    customers = pd.read_csv(DATA_DIR / "sample_customers.csv")
    return (
        customers.groupby("churn")
        .agg(rows=("customer_id", "size"), average_age=("age", "mean"))
        .reset_index()
    )


def main() -> None:
    print(demo_group_count().round(2).to_dict("records"))
