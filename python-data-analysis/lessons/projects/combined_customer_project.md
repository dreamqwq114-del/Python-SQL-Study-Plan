...
打开 [combined_customer_project.py](../../practice/projects/combined_customer_project.py)，按数据流一次只实现一个函数：
...
```powershell
python -m examples.projects.example_combined_customer_project
```

只测试综合项目：

```powershell
pytest tests/projects/test_combined_customer_project.py
```
