...
SELECT employee_name, salary FROM dbo.employees WHERE salary >= 15000;

-- 可运行示例 2：进行中或暂停的项目；结果 7 行。
SELECT project_name, project_status
FROM dbo.projects
WHERE project_status IN (N'进行中', N'暂停');

-- 可运行示例 3：姓名以“王”开头；结果 1 行。
SELECT employee_name FROM dbo.employees WHERE employee_name LIKE N'王%';

-- 可运行示例 4：没有结束日期的项目；结果 6 行。NULL 不是空字符串。
SELECT project_name, end_date FROM dbo.projects WHERE end_date IS NULL;

-- 旧知识复习
-- 题 1：查询工资最高的 2 名员工。返回 employee_name、salary；2 行。
-- 提示：TOP 配合 ORDER BY。TODO:
...
-- 错误 SQL：SELECT employee_name, department_id, salary FROM dbo.employees
