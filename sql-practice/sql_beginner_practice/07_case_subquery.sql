...
(
    SELECT department_id FROM dbo.departments WHERE city = N'上海'
);

-- 可运行示例 4：用 CASE 标记结束日期是否确定。
SELECT project_name,
       CASE WHEN end_date IS NULL THEN N'未确定' ELSE N'已确定' END AS end_date_status
FROM dbo.projects;

-- 旧知识复习
-- 题 1：找没有参与项目的员工；2 行。提示：LEFT JOIN。TODO:

-- 题 2：保留全部项目和经理姓名；14 行。提示：LEFT JOIN。TODO:
...
-- 错误 SQL：SELECT employee_name FROM dbo.employees
-- WHERE department_id = (SELECT department_id FROM dbo.departments);
-- 目标：查询上海部门的员工。请选择适合多值结果的运算符。TODO:

-- 题 10（改错）：下面 CASE 顺序会让 18000 也先命中“中”。
-- CASE WHEN salary >= 10000 THEN N'中'
--      WHEN salary >= 15000 THEN N'高' ELSE N'基础' END
