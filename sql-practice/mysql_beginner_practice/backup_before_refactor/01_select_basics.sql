...
2. 使用 LIMIT、DISTINCT 和 AS。
3. 写一个简单计算列。
*/

-- 基础语法（只看结构）
-- SELECT 列1, 列2 FROM 表;
-- SELECT 列 FROM 表 ORDER BY 排序列 LIMIT 数量;
-- SELECT DISTINCT 列 FROM 表;
-- SELECT 数值列 * 数字 AS 新列名 FROM 表;
-- ORDER BY 列 ASC/DESC：本课只用它让 LIMIT 结果稳定，第 3 天再系统练排序。
...
-- 结果必须稳定地按 project_id 最小的 5 个。提示：LIMIT 要配合 ORDER BY。
-- TODO:
