...
-- 修正：LEFT JOIN 保留无经理项目；IFNULL 只改变显示文字。
...
INNER JOIN projects AS p ON p.project_id = pm.project_id;

-- 题 11：先判断 NULL，再按高到低判断门槛，避免提前命中。
SELECT p.project_name,
       pm.investment_return_rate,
       CASE
           WHEN pm.investment_return_rate IS NULL THEN '未知'
           WHEN pm.investment_return_rate >= 0.0850 THEN '高'
           WHEN pm.investment_return_rate >= 0.0700 THEN '中'
           ELSE '低'
       END AS return_level
FROM project_metrics AS pm
INNER JOIN projects AS p ON p.project_id = pm.project_id;
...
SELECT p.project_name,
       d.department_name,
       IFNULL(e.employee_name, '未分配') AS manager_name,
       IFNULL(pc.participant_count, 0) AS participant_count,
       pm.management_fee,
       pm.investment_return_rate,
       pm.voltage_level,
       p.project_status
...
B. 用 SELECT COUNT(*) FROM projects; 得到 14，再确认最终查询也是 14 行。
