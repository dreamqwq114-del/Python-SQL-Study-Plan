...
-- 返回 department_id、average_salary；预计 3 行。
-- 提示：按 department_id 分组，再用 HAVING 筛选 AVG(salary)。TODO:
...
-- A. 下面写法缺少 ON，在 MariaDB/MySQL 中会直接报语法错误：
-- 错误 SQL：SELECT e.employee_name, d.department_name
--           FROM employees AS e INNER JOIN departments AS d;
-- B. 若写成 INNER JOIN departments AS d ON 1 = 1，虽然能运行，
--    却会得到 20×5=100 行组合。
...
-- 错误 SQL：SELECT e.employee_name, d.department_name
--           FROM employees AS e
--           INNER JOIN departments AS d ON d.department_id = e.employee_id;
